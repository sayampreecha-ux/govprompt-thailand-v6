(() => {
  ...
  const DOMAIN_TOOLS = Object.freeze([
    { id:'DOMAIN_FINANCE', icon:'💰', category:'การเงิน การคลัง และเบิกจ่าย', name:'วิเคราะห์การเงิน การคลัง และการเบิกจ่าย', desc:'ตรวจสิทธิ เงื่อนไข ฐานอำนาจ หลักฐาน และขั้นตอนด้านการเงินการคลังและการเบิกจ่าย', fields:['คำถามหรือเรื่องที่ต้องการตรวจสอบ','ข้อเท็จจริงที่มี','หน่วยงาน/ผู้เกี่ยวข้อง','ช่วงเวลา/วันที่เกี่ยวข้อง','เอกสารหรือหลักฐานที่มี'] },
  ...
  const DOMAIN_INTENTS = Object.freeze([
    { tool:'DOMAIN_FINANCE', pattern:/เบิก|เบิกจ่าย|การเงิน|การคลัง|คลัง|ค่าใช้จ่าย|ค่าตอบแทน|เงิน|ชำระ|จ่าย|ลูกหนี้|เจ้าหนี้|ฎีกา|ค่าเช่า|เช่าซื้อ|ค่า\s*k|เงินบำรุง/i },
  ...
  function queryTerms(query){
    ...
    const fragments=thaiRuns.flatMap(run=>{const terms=[];for(let length=4;length<=Math.min(run.length,8);length+=1){for(let start=0;start<=run.length-length;start+=1)terms.push(run.slice(start,start+length))}return terms});
    return unique([...words,...fragments]);
  }
  function scoreTool(terms,query,tool){
    const name=compact(tool.name),description=compact(tool.desc),category=compact(tool.category),fields=compact((tool.fields||[]).join(' '));
    const matchedTerms=terms.filter(term=>[name,description,category,fields].some(text=>text.includes(term)));
    const score=matchedTerms.reduce((total,term)=>total+(name.includes(term)?.12:0)+(description.includes(term)?.08:0)+(category.includes(term)?.04:0)+(fields.includes(term)?.03:0),0);
    const exactNameBonus=compact(query).includes(name)?.55:0;
  ...
  function domainCandidate(query){
    const match=DOMAIN_INTENTS.find(intent=>intent.pattern.test(query));
    if(!match)return null;
    const tool=DOMAIN_TOOLS.find(item=>item.id===match.tool);
    return tool?{tool,matchedTerms:['domain-intent'],score:.92}:null;
  }
  function route(query){
    ...
    const domain=domainCandidate(text);
    if(domain)ranked.push(domain);
    ...
    return Object.freeze({selectedGpId:selected.tool.id,score:Number(selected.score.toFixed(2)),confidence:Number(selected.score.toFixed(2)),matchedReason:`ตรงกับ ${selected.tool.id}: ${selected.matchedTerms.join(', ')}`,fallback:false,tool:selected.tool});
  }
