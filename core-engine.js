(() => {
  ...
  const AUTHORITY_CASE_PATTERN = /กฎหมาย|ระเบียบ|หลักเกณฑ์|หนังสือสั่งการ|มติ|สิทธิ|อำนาจ|เบิก|จ่าย|การเงิน|คลัง|พัสดุ|จัดซื้อ|งบประมาณ|ข้อบัญญัติ|แปรญัตติ|เดินทางไปราชการ|ค่าเช่า|เช่าซื้อ|งานก่อสร้าง|บุคคล|แต่งตั้ง|โอน|ย้าย|ราชการ/i;
  const AUTHORITY_CATEGORY_PATTERN = /กฎหมาย|การเงิน|คลัง|พัสดุ|งบประมาณ|บุคคล|สิทธิ|อำนาจ|สภา|เบิกจ่าย|เดินทาง|ช่าง|ราชการ/i;
  ...
  function extractSubject(query) {
    ...
    if (!subject || DEMONSTRATIVE_ONLY_PATTERN.test(subject) || GENERIC_ONLY_PATTERN.test(subject)) return '';
    return subject;
  }
  ...
  function buildSearchVocabulary({ query = '', discoveredIdentifiers = [] } = {}) {
    ...
    const subject = extractSubject(original);
  ...
  function evaluateOfficialAuthorityRetrieval({ query = '', discoveredIdentifiers = [] } = {}) {
    ...
    const subjectIdentifiable = Boolean(subject);
    return freeze({
      policyVersion: '7.1',
      subject,
      subjectIdentifiable,
  ...
  function assessDecisionSufficiency({ authoritySufficient, factsSufficient } = {}) {
    if (authoritySufficient !== true) {
      return freeze({
        ...
        nextAction: 'CONTINUE_AUTHORITY_RETRIEVAL'
      });
    }
    if (factsSufficient !== true) {
      return freeze({
  ...
  function shouldApplyOfficialAuthorityRetrieval({ query = '', category = '' } = {}) {
    ...
    return AUTHORITY_CATEGORY_PATTERN.test(c) || AUTHORITY_CASE_PATTERN.test(q);
  }
  ...
  function prepare(context) {
    ...
    const caseQuery = currentCaseText(source);
    const retrieval = evaluateOfficialAuthorityRetrieval({ query: caseQuery });
    return freeze({
      version: 7,
      coreRevision: '7.1',
      task: {
  ...
  function installOfficialAuthorityPromptPolicy() {
    ...
    form.onsubmit = function(event) {
      ...
      const policy = officialAuthorityPromptPolicy({ query: caseQuery, category: context.category || '' });
      if (policy && !output.textContent.includes('Official Authority Retrieval Gate — GovPrompt Thailand V7.1')) {
  ...
  if (typeof document !== 'undefined') {
    document.addEventListener('DOMContentLoaded', installOfficialAuthorityPromptPolicy, { once: true });
  }
