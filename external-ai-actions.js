(() => {
  ...
  function selectPrompt(){const output=document.getElementById('output'),selection=window.getSelection?.(),range=document.createRange?.();if(!output||!selection||!range)return;range.selectNodeContents(output);selection.removeAllRanges();selection.addRange(range)}
  ...
  async function openProvider(provider){const url=PROVIDERS[provider];if(!url||!getPrompt())return;const copied=await copyPrompt();const opened=window.open(url,'_blank','noopener,noreferrer');if(!opened){toast(copied?'คัดลอก Prompt แล้ว — กรุณาเปิด AI แล้ววางข้อความ':'เปิด AI ไม่สำเร็จ');return}toast(copied?'คัดลอก Prompt แล้ว — วางใน AI และกดส่งได้เลย':'เปิด AI แล้ว — กรุณาคัดลอก Prompt จากหน้านี้')}

  function ensureResultView(){
    if(resultView)return resultView;
    const view=document.createElement('section');
    view.id='gpResultView';view.hidden=true;
  ...
  function showResult(){
    ...
    document.body.style.overflow='hidden';view.hidden=false;view.scrollTop=0;return true;
  }
  function showHome(){const view=ensureResultView();view.hidden=true;document.body.style.overflow='';document.getElementById('modal')?.classList.add('hidden')}
