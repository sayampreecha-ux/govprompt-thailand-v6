(() => {
  ...
  function normalize(value) {
    return text(value).normalize('NFC').replace(/[()（）]/g, ' ').replace(/\\s+/g, ' ');
  }
  ...
  function detectFiscalYear(source = {}) {
    const input = normalize([source.fiscalYear, source.budgetYear, source.query, ...Object.values(source.userInputs || {})].join(' '));
  ...
  function evaluate(source = {}) {
    if (!isBudgetMatter(source)) {
      return Object.freeze({ applicable: false, status: 'NOT_APPLICABLE', decisionLock: false, blockers: [], orgType: detectOrgType(source), fiscalYear: detectFiscalYear(source) });
    }
    ...
    return Object.freeze({
      ...
      orgType,
      budgetUnitStatus,
      authorityBase: ['พ.ร.บ.วิธีการงบประมาณ พ.ศ. 2561', 'ระเบียบว่าด้วยการบริหารงบประมาณ พ.ศ. 2562', 'หลักเกณฑ์/หนังสือของสำนักงบประมาณที่ใช้บังคับในปีที่เกี่ยวข้อง', 'หนังสือจัดสรรของรายการ'],
      governorApproval
    });
  }
