-- Prevent duplicate assignment actions from mutating timestamps or creating misleading audit history.
...
begin
  ...
  if not (v_membership.role = 'ORG_ADMIN' or (v_membership.role = 'DIRECTOR' and v_membership.department_id = v_project.department_id)) then raise exception 'ROLE_FORBIDDEN'; end if;
  select * into v_target from public.organization_memberships
  where organization_id = v_project.organization_id and user_id = p_owner_user_id and active = true
    and department_id = v_project.department_id and role in ('DIRECTOR','OFFICER');
  ...
  if v_project.owner_user_id is not distinct from p_owner_user_id then return v_project; end if;
  update public.projects set owner_user_id = p_owner_user_id, last_updated_at = now(), updated_at = now()
  ...
  values(v_project.organization_id,v_project.department_id,v_user_id,'PROJECT_ASSIGNED','PROJECT',v_project.id::text,now(),p_request_id,
    jsonb_build_object('projectId',v_project.id::text,'changedFields',jsonb_build_array('ownerUserId'),'source','WORK_TRACKING_RPC'));
  return v_project;
end;
$$;
...
begin
  ...
  if not found then raise exception 'ROLE_FORBIDDEN'; end if;
  if not (v_membership.role = 'ORG_ADMIN' or (v_membership.role = 'DIRECTOR' and v_membership.department_id = v_task.department_id)) then raise exception 'ROLE_FORBIDDEN'; end if;
  select * into v_target from public.organization_memberships
  where organization_id = v_task.organization_id and user_id = p_assigned_user_id and active = true
    and department_id = v_task.department_id and role in ('DIRECTOR','OFFICER');
  ...
  if v_task.assigned_user_id is not distinct from p_assigned_user_id then return v_task; end if;
  update public.tasks set assigned_user_id = p_assigned_user_id, updated_at = now()
  where id = v_task.id returning * into v_task;
  insert into public.audit_events(organization_id,department_id,actor_user_id,action,entity_type,entity_id,occurred_at,request_id,metadata_json)
  values(v_task.organization_id,v_task.department_id,v_user_id,'TASK_ASSIGNED','TASK',v_task.id::text,now(),p_request_id,
    jsonb_build_object('taskId',v_task.id::text,'projectId',v_task.project_id::text,'changedFields',jsonb_build_array('assignedUserId'),'source','WORK_TRACKING_RPC'));
  return v_task;
end;
$$;
