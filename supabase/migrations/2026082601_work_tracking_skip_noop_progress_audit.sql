-- Keep Work Tracking audit history meaningful: a no-op update must not mutate timestamps or create an audit event.
...
begin
  ...
  insert into public.audit_events (
    organization_id, department_id, actor_user_id, action, entity_type, entity_id,
    occurred_at, request_id, metadata_json
  ) values (
    ...
    jsonb_build_object(
      ...
      'nextStatus', p_status,
      'previousProgress', v_previous_progress,
      'nextProgress', p_actual_progress,
      'problemChanged', (v_previous_problem is distinct from p_problem_summary),
