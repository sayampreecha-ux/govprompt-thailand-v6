-- Audited create operations for GP Work Tracking Pilot.
...
begin
  ...
  v_owner_id := coalesce(p_owner_user_id, case when v_membership.role = 'OFFICER' then v_user_id else null end);
  if v_membership.role = 'OFFICER' and v_owner_id <> v_user_id then raise exception 'OWNER_NOT_ALLOWED'; end if;
  ...
  if v_owner_id is not null then
    ...
    where organization_id = p_organization_id
      ...
      and active = true
      and (department_id = p_department_id or role = 'ORG_ADMIN');
...
begin
  ...
  v_assignee := coalesce(p_assigned_user_id, case when v_membership.role = 'OFFICER' then v_user_id else null end);
  if v_membership.role = 'OFFICER' and v_assignee <> v_user_id then raise exception 'ASSIGNEE_NOT_ALLOWED'; end if;
  ...
  if v_assignee is not null then
    ...
    where organization_id = v_project.organization_id
      ...
      and active = true
      and (department_id = v_project.department_id or role = 'ORG_ADMIN');
