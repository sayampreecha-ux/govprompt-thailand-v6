-- Atomic task mutations for GP Work Tracking Pilot.
...
begin
  ...
  if v_task.completed_at is distinct from p_completed_at then v_changed_fields := v_changed_fields || jsonb_build_array('completedAt'); end if;
...
begin
  ...
  where organization_id = v_task.organization_id
    ...
    and active = true
    and (department_id = v_task.department_id or role in ('ORG_ADMIN','EXECUTIVE'));
  if not found then raise exception 'ASSIGNEE_NOT_ALLOWED'; end if;

  if p_expected_updated_at is not null and v_task.updated_at <> p_expected_updated_at then raise exception 'CONFLICT_VERSION'; end if;

  update public.tasks
  set assigned_user_id = p_assigned_user_id, updated_at = now()
  where id = v_task.id
  returning * into v_task;
  ...
  insert into public.audit_events(
    ...
    v_task.organization_id, v_task.department_id, v_user_id, 'TASK_ASSIGNED', 'TASK', v_task.id::text, now(), p_request_id,
    jsonb_build_object(
      'taskId', v_task.id::text,
      'projectId', v_task.project_id::text,
